...
class ParNet(nn.Module):
	def __init__(self, in_channels, num_classes, block_c):
		...
		self.ds_1 = DownsampleBlock(in_channels, block_c[0])
		self.ds_2 = DownsampleBlock(block_c[0], block_c[1])
		self.ds_3 = DownsampleBlock(block_c[1], block_c[2])
		self.ds_4 = DownsampleBlock(block_c[2], block_c[3])

		self.stream1 = nn.Sequential(
			Stream(block_c[1], 4),
			DownsampleBlock(block_c[1], block_c[2])
		)

		self.stream2 = Stream(block_c[2], 5)
		self.fusion1 = FusionBlock(block_c[2], block_c[3])

		self.stream3 = Stream(block_c[3], 5)
		self.fusion2 = FusionBlock(block_c[3], block_c[3])

		self.ds_final = DownsampleBlock(block_c[3], block_c[4])
