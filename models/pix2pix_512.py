...
class Generator(nn.Module):
	def __init__(self):
		...
		# Encoder
		self.encoder_1 = nn.Conv2d(3, 64, kernel_size=4, stride=2, padding=1)
		self.encoder_2 = nn.Conv2d(64, 64, kernel_size=4, stride=2, padding=1)
		self.e_batcn_2 = nn.BatchNorm2d(64)
		self.encoder_3 = nn.Conv2d(64, 128, kernel_size=4, stride=2, padding=1)
		self.e_batcn_3 = nn.BatchNorm2d(128)
		self.encoder_4 = nn.Conv2d(128, 256, kernel_size=4, stride=2, padding=1)
		self.e_batcn_4 = nn.BatchNorm2d(256)
		self.encoder_5 = nn.Conv2d(256, 512, kernel_size=4, stride=2, padding=1)
		...
		self.encoder_8 = nn.Conv2d(512, 512, kernel_size=4, stride=2, padding=1)
		self.e_batcn_8 = nn.BatchNorm2d(512)
		...
		self.decoder_7 = nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1)
		self.dropout_7 = nn.Dropout(p=0.5)
		self.decoder_8 = nn.ConvTranspose2d(128, 3, kernel_size=4, stride=2, padding=1)
	...
	def forward(self, x):
		...
		x_e8 = F.leaky_relu(self.e_batcn_8(self.encoder_8(x_e7)), negative_slope=0.2)

		x_d8 = F.relu(self.dropout_1(self.decoder_1(x_e8)))
		x_d8 = torch.cat((x_e7, x_d8), dim=1)
		x_d7 = F.relu(self.dropout_2(self.decoder_2(x_d8)))
		x_d7 = torch.cat((x_e6, x_d7), dim=1)
		x_d6 = F.relu(self.dropout_3(self.decoder_3(x_d7)))
		x_d6 = torch.cat((x_e5, x_d6), dim=1)
		x_d5 = F.relu(self.dropout_4(self.decoder_4(x_d6)))
		x_d5 = torch.cat((x_e4, x_d5), dim=1)
		x_d4 = F.relu(self.dropout_5(self.decoder_5(x_d5)))
		x_d4 = torch.cat((x_e3, x_d4), dim=1)
		x_d3 = F.relu(self.dropout_6(self.decoder_6(x_d4)))
		x_d3 = torch.cat((x_e2, x_d3), dim=1)
		x_d2 = F.relu(self.dropout_7(self.decoder_7(x_d3)))
		x_d2 = torch.cat((x_e1, x_d2), dim=1)
		x_d1 = torch.tanh(self.decoder_8(x_d2))
